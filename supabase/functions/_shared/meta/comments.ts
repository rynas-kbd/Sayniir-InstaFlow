...
import { TokenExpiredError } from './api.ts'
import { decryptApiKey, isEncrypted } from '../crypto.ts'

const GRAPH_API_VERSION = 'v21.0'

/**
 * Shape of an `automation_rules` row as consumed by comment auto-reply matching.
 * Only the fields read in this file are declared.
 */
...
export async function handleCommentAutoReply({
  ...
  const applicableRules = rules?.filter((rule: AutomationRule) => {
  ...
  applicableRules.sort((a: AutomationRule, b: AutomationRule) => {
  ...
  let matchedRule: AutomationRule | undefined
  ...
  if (applicableRules.length > 0) {
    ...
    if (!matchedRule) {
      matchedRule = applicableRules.find(
        (rule: AutomationRule) => rule.trigger_type === 'any_comment'
      )
    }
  }
