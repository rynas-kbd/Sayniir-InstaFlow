...
describe('validateRuleForm', () => {
  test('requires an account and a name', () => {
    const errors = validateRuleForm(form({ channel_account_id: '', name: '  ' }), 'dm')
  ...
  test('trigger_keywords required only for keyword-based trigger types (kind-aware)', () => {
    expect(validateRuleForm(form({ trigger_type: 'any_message', trigger_keywords: [] }), 'dm').trigger_keywords).toBeUndefined()
    expect(validateRuleForm(form({ trigger_type: 'keyword', trigger_keywords: [] }), 'dm').trigger_keywords).toBeTruthy()
    expect(validateRuleForm(form({ trigger_type: 'keyword', trigger_keywords: ['hi'] }), 'dm').trigger_keywords).toBeUndefined()
  })

  test('response_text required for a pure DM rule, not required once switched to a card', () => {
    expect(validateRuleForm(form({ response_type: 'text', response_text: '' }), 'dm').response_text).toBeTruthy()
    expect(validateRuleForm(form({ response_type: 'card', response_text: '', card_title: 'T' }), 'dm').response_text).toBeUndefined()
  })
  ...
  test('comment tab: response_text is required across comment/dm/both — a rule always needs a message', () => {
    ...
    expect(validateRuleForm(commentForm(), 'comment').response_text).toBeTruthy()
    expect(validateRuleForm(commentForm({ reply_method: 'dm' }), 'comment').response_text).toBeTruthy()
    expect(validateRuleForm(commentForm({ reply_method: 'both' }), 'comment').response_text).toBeTruthy()
  })
  ...
  test('card fields validated only when isCard (kind-aware)', () => {
    expect(validateRuleForm(form({ response_type: 'text', card_title: '' }), 'dm').card_title).toBeUndefined()
    expect(validateRuleForm(form({ response_type: 'card', card_title: '' }), 'dm').card_title).toBeTruthy()
  })
