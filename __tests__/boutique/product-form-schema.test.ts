...
// Identity translator — tests only assert error presence/absence, never message text,
// so falling back to the dot-path key (the translator's built-in missing-key behavior) is fine.
...
describe('validateProductForm', () => {
  test('empty name and empty/invalid price are flagged', () => {
    const errors = validateProductForm(form({ name: '  ', price: '' }), t)
  ...
  test('is kind-aware: an invalid capacity from a previous event kind stops being validated after switching to physical', () => {
    ...
    const errors = validateProductForm(f, t)
    expect(errors.capacity).toBeUndefined()
  })

  test('event kind validates capacity and event_date', () => {
    const errors = validateProductForm(form({ kind: 'event', capacity: '0', event_date: '26/08/2026' }), t)
    expect(errors.capacity).toBeTruthy()
    expect(errors.event_date).toBeTruthy()
  })

  test('currency must be exactly 3 uppercase letters', () => {
    expect(validateProductForm(form({ currency: 'dzd' }), t).currency).toBeTruthy()
    expect(validateProductForm(form({ currency: 'DZ' }), t).currency).toBeTruthy()
    expect(validateProductForm(form({ currency: 'DZD' }), t).currency).toBeUndefined()
  })

  test('image_url and digital file_url must be http(s) when non-empty, but are optional', () => {
    expect(validateProductForm(form({ image_url: '' }), t).image_url).toBeUndefined()
    expect(validateProductForm(form({ image_url: 'not-a-url' }), t).image_url).toBeTruthy()
    expect(validateProductForm(form({ image_url: 'https://x.test/i.png' }), t).image_url).toBeUndefined()
    expect(validateProductForm(form({ kind: 'digital', file_url: 'ftp://x.test/f' }), t).file_url).toBeTruthy()
  })

  test('valid minimal physical form has no errors', () => {
    expect(validateProductForm(form(), t)).toEqual({})
  })
})
