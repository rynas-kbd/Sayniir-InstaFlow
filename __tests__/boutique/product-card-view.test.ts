...
describe('deriveOptionBadges', () => {
  test('physical: concatenates sizes then colors, in order', () => {
    const badges = deriveOptionBadges({ kind: 'physical', sizes: ['S', 'M'], colors: ['Noir'], metadata: {} })
    expect(badges).toEqual(['S', 'M', 'Noir'])
  })

  test('service: duration then location, each optional', () => {
    expect(deriveOptionBadges({ kind: 'service', sizes: [], colors: [], metadata: { duration_minutes: 30 } })).toEqual(['30 min'])
    expect(
      deriveOptionBadges({ kind: 'service', sizes: [], colors: [], metadata: { duration_minutes: 30, location: 'En ligne' } }),
    ).toEqual(['30 min', 'En ligne'])
  })

  test('subscription: monthly vs yearly label', () => {
    expect(deriveOptionBadges({ kind: 'subscription', sizes: [], colors: [], metadata: { billing_period: 'monthly' } })).toEqual([
      'Facturation mensuelle',
    ])
    expect(deriveOptionBadges({ kind: 'subscription', sizes: [], colors: [], metadata: { billing_period: 'yearly' } })).toEqual([
      'Facturation annuelle',
    ])
  })

  test('event: remaining=0 still surfaces "0 places restantes" (uses != null, not truthiness)', () => {
    ...
    expect(badges).toEqual(['2026-08-01', '0 places restantes'])
  })

  test('event: remaining undefined omits the badge entirely', () => {
    const badges = deriveOptionBadges({ kind: 'event', sizes: [], colors: [], metadata: { event_date: '2026-08-01' } })
...
describe('productToCardView', () => {
  test('maps a saved product 1:1, deriving badges from its own kind/sizes/colors/metadata', () => {
    const view = productToCardView(product({ sizes: ['S'], colors: ['Blanc'] }))
    expect(view).toEqual({
      kind: 'physical',
      name: 'T-shirt',
      description: null,
...
describe('formToCardView', () => {
  test('empty form yields placeholders: empty name, null price', () => {
    const view = formToCardView(emptyForm())
  ...
  test('coerces price string to number and trims name/description/image_url', () => {
    ...
    const view = formToCardView(form)
  ...
  test('event preview reuses buildMetadata so remaining can never diverge from the saved payload', () => {
    ...
    const view = formToCardView(form, savedEvent)
