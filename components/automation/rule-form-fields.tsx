...
export interface RuleFormFieldsProps {
  ...
  /** Called after onSave resolves successfully — e.g. close the dialog, or navigate away. */
  onSaved?: () => void
  onCancel?: () => void
  submitLabel?: string
...
/** Shared fields for both RuleFormDialog (edit) and the /automation/new page (create). */
export function RuleFormFields({ accounts, rule, defaultTab, onSave, onSaved, onCancel, submitLabel, onPostSelectorToggle }: RuleFormFieldsProps) {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      ...
      <div className="mt-1 flex gap-2">
        ...
        <Button type="submit" className="flex-1" disabled={saving || selectableAccounts.length === 0}>
          {saving ? 'Enregistrement…' : submitLabel ?? (rule ? 'Sauvegarder' : 'Créer la règle')}
        </Button>
      </div>
    </form>
  )
