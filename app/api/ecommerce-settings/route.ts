...
import { propagateAgentSettings } from '@/lib/boutique/sync'
...
export async function GET(request: NextRequest) {
  ...
  if (!settings || error?.code === 'PGRST116') {
    return NextResponse.json({
      ...
      is_availability_check_active: false,
      instructions: [],
      infos_to_collect: [],
...
export async function PATCH(request: NextRequest) {
  ...
  const { data: settings, error } = await supabase
    ...
    .upsert({
      ...
      is_availability_check_active: is_availability_check_active ?? existing?.is_availability_check_active ?? false,
      instructions: instructions ?? existing?.instructions ?? [],
      infos_to_collect: infos_to_collect ?? existing?.infos_to_collect ?? [],
      ai_provider: ai_provider ?? existing?.ai_provider ?? 'openrouter',
      ai_api_key: finalApiKey,
      ai_model: ai_model ?? existing?.ai_model ?? 'nvidia/nemotron-3-ultra-550b-a55b:free',
      ...
      vertical_config: vertical_config ?? existing?.vertical_config ?? {},
